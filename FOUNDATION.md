...
第一阶段已完成工程规则和文档，第二阶段已完成可执行工程骨架。当前第三阶段只建立 tenant-aware PostgreSQL 持久化与最小 Project 领域；仍不得实现 Brief、AI、认证 Provider、生产队列、云资源或自动成片功能。持久化决定由 [ADR-012 至 ADR-016](docs/adr/) 记录，属于可替换实现假设，不改变下列冻结决定。
