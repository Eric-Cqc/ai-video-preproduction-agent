...
_OLD_OUTCOME = (
    ...
    "AND outcome_delivery_package_id IS NOT NULL AND outcome_delivery_package_version_id IS NOT NULL "
...
_NEW_OUTCOME = (
    ...
    "IS NULL AND outcome_delivery_package_id IS NULL AND outcome_delivery_package_version_id IS NULL "
    "AND outcome_export_file_id IS NULL)"
)
_OLD_AUDIT = (
...
def upgrade() -> None:
    op.add_column("delivery_operations", sa.Column("failure_code", sa.String(length=40), nullable=True))
...
def downgrade() -> None:
    ...
    failed_operations = int(
        bind.scalar(
            sa.text("SELECT count(*) FROM delivery_operations WHERE status = 'failed'")
        )
        or 0
    )
    failed_audits = int(
