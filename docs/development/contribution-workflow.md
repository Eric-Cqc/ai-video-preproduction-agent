...
4. Project/Brief mutation 必须携带 tenant scope、expected version（适用时），并与最小 AuditEvent 在一个 UoW 中提交。Ingestion 必须先 PostgreSQL reservation，再受限 finalize；不得提交长期 `reserved`。
