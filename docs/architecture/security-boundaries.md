...
- SourceAsset intake 是 metadata-only：没有上传、object storage、文件字节、checksum verification 或 MIME sniffing。SourceAssetVersion 的 SHA-256 与 byte size 都是声明值；filename 不是路径，source provenance 拒绝路径、database URL、credential/Bearer/token、signed URL 与控制字符，且不写入 Audit。
...
本阶段主要在应用层隔离，未启用 PostgreSQL RLS；数据库管理员也能直接修改 immutable-version 或 audit table。Content-Length 检查也不能替代反向代理/ASGI server 的传输层限制。来源 metadata 不能证明文件实际存在、大小、类型或 checksum，也不是安全的外部 URL retrieval 通道。首次共享生产、外部协作者或受监管数据前，必须新增 auth Adapter、威胁模型和基础设施 body limit，并复审 RLS、数据库角色、凭据轮换、备份恢复、审计防篡改和保留策略。
